...
- [[4.02 序列化]] - `requests.get(...).json()` 背后是 `json.loads`
- [[1.05 字符编码]] - `r.text` 自动检测编码，`r.content.decode(...)` 手动
- [[5.11 XML 与 HTML 解析]] - 拿到 HTML 后怎么解析
- [[4.03 并发编程]] - 多个 HTTP 请求并发（线程 / async）
- [[0.02 环境变量与配置]] - API key 应该放环境变量
