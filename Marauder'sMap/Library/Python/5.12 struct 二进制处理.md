...
| 进程间传 Python 对象 | ✗ 用 [[4.02 序列化]] 里的 pickle |
| 给前端送 API 数据 | ✗ 用 JSON |
...
- [[1.05 字符编码]] - `bytes` 和 `str` 的区别（struct 永远操作 bytes）
- [[4.01 IO 编程]] - 二进制文件读写 `open(path, 'rb')`
- [[5.09 密码学工具]] - 哈希算法的输入也是 bytes，常与 struct 配合
- [[4.02 序列化]] - 高层的对象 ↔ bytes 转换
