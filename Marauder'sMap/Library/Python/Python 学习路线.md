...
学完 Stage 3 即可并行开始 [[AI开发学习计划]] 的第 2 周内容（FastAPI + Prompt 工程）。
...
> **里程碑：读懂公司数据生成项目的 LLM 封装模块 `manager.py`（约 400 行，《AI开发学习计划》推荐的最简入口）。**
...
vault 根目录的 [[AI开发学习计划]] 是「4 周上手公司项目」的总纲，本路线是其 **第 1 周（Python 基础）的展开版**：

```text
本路线 Stage 0-3  ≈  AI开发学习计划 第 1 周（Python 基础 + LLM API 入门）
       Stage 4    ≈  第 2 周的 FastAPI 部分
       ────────────────────────────────────────
       Stage 3 完成后 → 即可并行开始第 2 周（Prompt 工程、数据生成项目实操）
       Stage 4 完成后 → 进入第 3 周（LangChain + MCP + Agent）
```

碎片时间下不必追求「学完再上手」：**Stage 3 一过就去跑公司的数据生成项目**，读不懂的地方回头补对应笔记，比线性刷完效率高。
...
- [[AI开发学习计划]] —— 公司项目全景分析 + 4 周计划 + JS/Python 核心对照表
