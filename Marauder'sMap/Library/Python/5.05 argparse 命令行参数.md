...
- [[1.12 模块与包]] - `if __name__ == '__main__'` 配合 CLI 入口
- [[0.02 环境变量与配置]] - 命令行 + 环境变量 + 配置文件三层组合
- [[2.02 异常处理]] - argparse 内部用 `SystemExit`，普通 `except` 抓不到
- [[5.03 collections 高级容器]] - `ChainMap` 实现配置优先级查找
