...
> [!warning] 阅读前
> 全文会反复出现四类数字：**超参数**（4096、32 层）、**参数**（权重矩阵里的值）、**输入特征**、**激活值**。分不清就会处处卡壳——先读 [[模型里的四类数字]]，或至少记住第 1.5 节的速记。
...
### 1.2 为什么是 .safetensors

这个后缀拆开是 **safe + tensors**，两个词各回答一半问题。

**tensors：张量就是"任意维数组"**

```text
0 维 → 标量   3.14
1 维 → 向量   [1, 2, 3]              ← LayerNorm 权重是这个
2 维 → 矩阵   [[1, 2], [3, 4]]       ← W_q、embedding 表是这个
3 维 → ...    批量 × 序列 × 维度      ← 批量推理时的激活值
```

**张量是这一整套的统称**，不是到第 3 维才叫张量。

那为什么不叫 `.matrices`？因为下一节数出来的 226 个是矩阵，但文件里还躺着 65 个一维的归一化权重——**一共 291 个张量**。深度学习框架懒得按维数分家，统一用 tensor 指"n 维数组"，PyTorch 的核心类型就叫 `torch.Tensor`。

> 一个容易踩的坑：数学和物理里 tensor 有严格定义（满足特定变换规则的多重线性映射）。机器学习**借了这个词但抛弃了那层含义**，就只是多维数组。读 [[线性代数入门]] 时碰到严格定义的张量，不必与这里对齐。

**safe：格式里没有能放代码的地方**

这个格式是 HuggingFace 2022 年造的，用来替换 PyTorch 原来的 `.bin` / `.pt`。老格式用 Python 的 **pickle** 序列化，而 pickle 在反序列化时会调用对象的 `__reduce__`，那里可以指定**任意一个可调用对象**。翻译成后果：

```python
torch.load("从网上下的模型.bin")   # 等于执行了陌生人的代码
```

不是理论风险，Hub 上真出现过藏后门的权重文件。而权重恰恰是最容易让人放松警惕的东西——大家心里觉得"这就是一堆数字"。

safetensors 的解法很朴素：**格式里没有能表达"代码"的位置**。整个文件只有三段：

```mermaid
flowchart LR
    A["8 字节<br/>小端 u64<br/>= header 长度 N"] --> B["N 字节 JSON header<br/>每个张量的<br/>名字 / dtype / 形状 / 偏移量"] --> C["剩下全是裸字节<br/>纯数值，无结构"]
```

解析它只需要"读长度 → 解 JSON → 按偏移切片"，没有任何可执行语义。

**拿一个真文件看一眼**

以 `BAAI/bge-small-zh-v1.5` 为例（[[Embedding Service]] 里那张表的小号模型），读出它的 header：

```python
import json, struct

with open("model.safetensors", "rb") as f:
    n = struct.unpack("<Q", f.read(8))[0]   # 前 8 字节：header 有多长
    header = json.loads(f.read(n))          # 接下来 n 字节：JSON
```
...
embeddings.word_embeddings.weight       [21128, 512]   F32   offsets=[1060864, 44331008]
embeddings.position_embeddings.weight   [512, 512]     F32   offsets=[8192, 1056768]
embeddings.LayerNorm.weight             [512][[          ]]F32   offsets=[6144, 8192]
```

对一下账：`21128 × 512 × 4 字节（F32）= 43,270,144`，而 `44331008 − 1060864 = 43,270,144`，**严丝合缝**。格式就这么直白——JSON 里写着"我在第几字节到第几字节"，按那个区间切出来、reshape 成声明的形状，就是那个张量。

`dtype` 这一栏也是 14 GB 的来源：7B 模型一般存成 BF16 或 F16，每个数 2 字节，70 亿 × 2 = 14 GB。同一个模型存成 F32 就是 28 GB，量化到 4 bit 约 3.5 GB。**参数量和文件大小是两回事，中间隔着 dtype。**

顺带留意 `[21128, 512]` 这一行：它就是第 3 节要查的 embedding 表。同目录下的 `vocab.txt` 正好 **21128 行**——分词器与 embedding 表靠行号对齐这件事（见第 3.2 节），在这里能直接数出来。

**两个顺带的好处**

- **加载快**：数据是连续的裸字节，可以 `mmap` 直接映射进内存，**零拷贝**。pickle 那套要先在内存里重建 Python 对象再搬进显存，峰值内存翻倍——14 GB 的模型，这个差别很实在。
- **能按需取**：header 里每个张量的偏移量互相独立，只想读 `lm_head.weight` 就只读那一段。分片模型（`model-00001-of-00003.safetensors`）和多卡切分都依赖这个特性。

还有个副作用：格式不绑 Python。官方实现是 Rust 写的，Rust / C++ / JS 都能读，`.bin` 做不到。

（PyTorch 2.6 起 `torch.load` 默认 `weights_only=True`，算是在 API 层堵了一半；但格式层面的根治仍是换成 safetensors。另一个常见格式是 llama.cpp 那边的 `.gguf`，把配置和词表也一起打包进单文件，思路不同但同样不含代码。）

一句话收口：**`model.safetensors` 是个只装数字、不装代码的容器**。它自己不知道这 291 个张量该怎么用——那是 `config.json` 那份说明书的活。

### 1.3 打开 safetensors：226 个矩阵
...
分词器拿着一张 32000 行的词表，**贪心取最长匹配**，把字节流切成 token：
...
「我」的 3 个字节被**一个** token 整块吞掉（词表里有「我」这一行）；「藜」不在词表里，触发**字节兜底**——3 个字节**各自**成为一个 token。`<0xE8>` 这类 token 是词表专门为 256 个字节值预留的行，覆盖全部可能的字节（2⁸ 穷尽），所以编码层面永远不会"不认识"。

注意**不是每个字节都切一个 token**：5 个字在 UTF-8 层面都是 3 字节、待遇平等，差别只在词表里有没有专属行。这也是中文比英文费 token 的原因：词表以英文为主，汉字经常退化到字节级。词表为什么是 32000 行、为什么 256 行就够兜底，完整机制见 [[Token 与 Embedding 表]]。
...
```text
 我     爱     吃    <0xE8> <0x97> <0x9C>   麦
1234   5678   910    235    154    159    4321
```

每个数是 token 在词表里的**行号**。此时编号的大小没有任何含义（4321 不比 910 "大"），不能直接参与计算——所以需要下一步。

三个字节 token 的编号落在 **3~258** 这个号段，那是词表给 256 个字节值预留的位置（`<0x00>` = 3，所以 `<0xE8>` = 3 + 0xE8 = 235）；常用字的编号散落在 259 以后。真实分词器还会在最前面补一个 `<s>`（id 1）标记序列起点，这里略去。
...
embedding 表是一张 32000 × 4096 的大表（第 1.3 节清单里的第一个矩阵）。拿上一步的 7 个编号，**按行号取出对应的 7 行**：

```text
[1, 1234, 5678, ...]  → 查表取行 →  7 个 4096 维向量，拼成 7 × 4096 的矩阵
```

**行数 32000 不是自由选的——必须等于词表行数**，一一对应：每个 token 得有属于自己的一行，多一行是死代码，少一行就有 token 查不到表。词表大小 32000 是个超参数（[[模型里的四类数字]]），它同时决定了这张表和末尾 `lm_head` 的尺寸——两个矩阵合计 2.6 亿参数，占 7B 的 3.9%。这也是词表不能随便调大的主因，权衡见 [[Token 与 Embedding 表]]。
...
- **不额外花钱。** $W_q$ 仍是一个 4096 × 4096 的矩阵，只是被按列切成 32 块分给各头；计算量也一样（32 次 128 维点积 = 1 次 4096 维点积）。**同等成本把 1 个注意力分布换成 32 个**，这才是多头的意义。
- **分工没人指定。** 每个头把输入投影到自己那个 128 维子空间，子空间不同 → "什么算相似"的标准不同 → 关注对象不同。分化是训练逼出来的：两个头学成一样就是冗余，梯度会推动它们分开。
- **实际分工没那么整齐。** 确实有功能明确的头（如 induction head），但多数头没有清晰单一功能，且剪枝实验显示大量头可以删掉而性能几乎不降。

机制细节、头数为什么是 32、以及 GQA（Q 和 K/V 头数可以不同，用来压缩 KV 缓存）见 [[Transformer]] 第 7 节。
...
每层前馈约 1.35 亿参数，注意力约 6700 万——**前馈是注意力的两倍**。注意力更出名，但每层的参数和算力大头其实在前馈那三个大矩阵上（第 1.4 节的分布表也印证了这点）。
...
上面那一整块原样重复 32 次，**结构完全相同，参数各不相同**（每层有自己的 Q/K/V/O 和前馈权重——这就是第 1.3 节参数量要乘 32 的原因）：
...
| 分不清 4096 是参数还是维度 | 4096 是**尺寸**（超参数），参数是填在那 4096 列里的数值（第 1.5 节） |
...
| 站点 | 知识点 | 详细笔记 |
|---|---|---|
| 第 1 节 模型家底 | 四个文件、safetensors 格式、226 个矩阵、四类数字 | [[模型里的四类数字]] |
